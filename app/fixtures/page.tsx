...
export default function FixturesPage() {
  ...
  async function loadFixtures() {
    setLoading(true)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/fixtures/${date}`)
      if (res.ok) {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
            ...
            <p className="text-sm mt-2">Start the backend to fetch fixtures from scrapers</p>
          </div>
        ) : (
